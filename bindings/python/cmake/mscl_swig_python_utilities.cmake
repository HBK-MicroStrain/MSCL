macro(mscl_add_swig_python_module_library MSCL_PYTHON_VERSION MSCL_PYTHON_MAJOR_VERSION)
    # When ON, install to the package root (wheel-compatible layout, matching how the abi3
    # macro below installs) instead of the default site-packages-shaped path. Used for
    # per-version wheels (e.g. free-threaded builds) that scikit-build-core packages directly
    # from the install tree, where a site-packages-shaped path would land in the wrong place.
    option(MSCL_PYTHON_WHEEL_INSTALL_LAYOUT
        "Install the non-abi3 Python bindings to the package root (wheel-compatible) instead of a site-packages-shaped path"
        OFF
    )
    ...
    # CMake version arguments must be numeric dot-separated (major[.minor[.patch]]) -- the
    # free-threaded "t" suffix is an ABI flag, not part of the interpreter's version number,
    # so it has to be stripped before it's used as a find_package version constraint. Which
    # interpreter actually gets found (free-threaded or not) is still controlled correctly:
    # Python3_EXECUTABLE/PATH is expected to already point at the intended one (e.g. via
    # actions/setup-python's python-version: '3.13t' in CI/CD).
    string(REGEX REPLACE "t$" "" MSCL_PYTHON_FIND_VERSION "${MSCL_PYTHON_VERSION}")

    # CMake won't look for a free-threaded interpreter's Development component unless
    # told to -- Python3_FIND_ABI's 4th element (gil_disabled) defaults OFF on Windows,
    # which is why find_package below can find the interpreter itself but then report
    # Development/Development.Module/Development.Embed as missing for it.
    if(MSCL_PYTHON_VERSION MATCHES "t$")
        set(Python3_FIND_ABI "OFF" "OFF" "OFF" "ON")
    endif()

    # Find the required Python package
    find_package("${PYTHON_PACKAGE_NAME}"
        "${MSCL_PYTHON_FIND_VERSION}" EXACT
        REQUIRED
    ...
    # The Windows Python installer ships one pyconfig.h shared between the regular and
    # free-threaded ("t") executables, configured for the regular one -- so Py_GIL_DISABLED
    # never gets auto-defined when building against the free-threaded interpreter, and
    # Python.h's auto-link pragma falls back to requesting the wrong (non-"t") import lib.
    # Defining it ourselves is the documented workaround (see the CPython core devs'
    # discussion: https://discuss.python.org/t/windows-installer-freethreading-and-building-extension-modules/54391).
    if(WIN32 AND MSCL_PYTHON_VERSION MATCHES "t$")
        target_compile_definitions("${MSCL_PYTHON_TARGET_NAME}" PRIVATE Py_GIL_DISABLED=1)
    endif()
    ...
    if(MSCL_PYTHON_WHEEL_INSTALL_LAYOUT)
        set(MSCL_PYTHON_INSTALL_DESTINATION ".")
    else()
        set(MSCL_PYTHON_INSTALL_DESTINATION "${MSCL_PYTHON_SITE_PACKAGES_DIR}")
    endif()
    ...
    install(
        ...
        RUNTIME
            ...
            DESTINATION "${MSCL_PYTHON_INSTALL_DESTINATION}"
        LIBRARY
            COMPONENT "${MSCL_PYTHON_COMPONENT_NAME}"
            DESTINATION "${MSCL_PYTHON_INSTALL_DESTINATION}"
    )
...
macro(mscl_add_swig_python_abi3_module_library MSCL_PYTHON_ABI_VERSION)
    ...
    if(UNIX AND NOT APPLE)
        # vcpkg always builds Linux dependencies against the dynamic triplet (see
        # cmake/mscl-configure-vcpkg.cmake), regardless of MSCL_LINK_STATIC_DEPS, so only
        # shared Python libraries are ever installed here. This only affects what find_package
        # accepts to satisfy the Development component below -- Python3_LIBRARIES itself is
        # never linked on Linux (see the note above), so this doesn't change the built artifact.
