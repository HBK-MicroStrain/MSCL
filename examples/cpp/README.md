...
**Note:** The wireless example has moved to a walkthrough guide: see
[wireless/CPP.md](wireless/CPP.md).
...
```powershell
cmake --build build --config Release --target MSCL-Cpp-Inertial-Example --parallel $env:NUMBER_OF_PROCESSORS
```

#### Linux

```shell
cmake --build build --target MSCL-Cpp-Inertial-Example --parallel $(nproc)
```
