...
namespace mscl
{
    ...
    protected:
        //Variable: m_commMutex
        //    Serializes access to m_comm (and the connect/disconnect state it depends on)
        //    so a reconnect (e.g. triggered by a baud rate correction) on one thread can't
        //    tear down m_comm while another thread is mid-command on it.
